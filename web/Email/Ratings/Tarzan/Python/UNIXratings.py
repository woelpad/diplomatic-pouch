...
def Read_Game_Summaries ( Directory, First_Game_Date, Pot, Powers ) :
	for Judge in os.listdir( Directory ) :
		if Debug_Flag == 'ON' :
			print 'Reading in the Game Summaries from the %s Judge' % Judge
		for File_Name in os.listdir( Directory + '/' + Judge ) :
			if Debug_Flag == 'ON' :
				print '     Reading in the %s Judge Game Summary from file < %s >' % ( Judge, File_Name )
			( Judge_Name, Game_Name, Game_Press, Game_Start, Game_Finish, Game_End_Date, Game_Players, Game_Winners, Game_Eliminees ) = judge_summary.Read_Game( Directory + '/' + Judge + '/' + File_Name, First_Game_Date, Powers )
...
#
	for Dir_Name in os.listdir( Directory ) :
		if Debug_Flag == 'ON' :
			print 'Reading in the Game Summaries from the %s Judge' % Dir_Name
